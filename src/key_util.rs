...
pub fn get_public_key_from_private_key_primitive(key: u128) -> PublicKey {
    let mut k: [u8; 32] = [0; 32];
    k[16..].copy_from_slice(&key.to_be_bytes());
    get_public_key_from_private_key(k)
}

pub fn get_public_key_from_private_key_vec(key: Vec<u8>) -> PublicKey {
    let mut num: [u8; 32] = [0; 32];
    let key_slice = key.as_slice();

    let start_index = 32 - key_slice.len();
    num[start_index..32].copy_from_slice(key_slice);
    get_public_key_from_private_key(num)
}

pub fn get_public_key_from_private_key(key: [u8; 32]) -> PublicKey {
    let secret_key = SecretKey::from_secret_bytes(key).expect("32 bytes, within curve order");
    PublicKey::from_secret_key(&secret_key)
}

pub fn get_public_key_from_secret_key(secret_key: SecretKey) -> PublicKey {
    PublicKey::from_secret_key(&secret_key)
}
    ...
    fn verify_negation() {
        let private_key =
            SecretKey::from_str("000000000000000000000000000000000000000000000000f7051f27b09112d4")
                .unwrap();
        let public_key = get_public_key_from_secret_key(private_key);
        assert_eq!(
        ...
        assert_eq!(
            negated_private_key.to_secret_bytes(),
            hex!("fffffffffffffffffffffffffffffffebaaedce6af48a03ac8cd3f651fa52e6d")
        );

        let negated_public_key = public_key.negate();
        assert_eq!(
        ...
        assert_eq!(public_key, negated_public_key.negate());

        let x = public_key.x_only_public_key();
        let uint = BigUint::from_bytes_be(&x.0.to_byte_array());
